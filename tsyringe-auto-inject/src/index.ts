...
class AutoInjiect {
  ...
  constructor(options: AutoInjiectOptions) {
    ...
    this.sourceFileList = this.project.addSourceFilesAtPaths([
      "./src/domain/**/*.ts",
      ...
      for (const classe of classList) {
        ...
        if (isAbstract(classe) && classe.isExported()) {
          ...
          if (!this.ignoreAbstractList.includes(abstractName)) {
            ...
            const implementsInFile = this.isImplementsInFile(
              classList,
              abstractName
            );
            if (implementsInFile) {
              const { classSourceFile, abstractName } = implementsInFile;
              this.addReferenceAbstract(
                abstractName,
                abstractName,
                abstractName
              );
              ...
              const referencesourceFile = this.getReferenceSourceFile(
                abstractClass,
                abstractName
              );
  ...
  getRealImportAbstractName(
    ...
    for (const item of importDeclaration
      ...
      if (
        ...
        exportAbstractName === spaceReplace(importName[0].getFullText())
      ) {
        const importAbstractName =
          importName.length == 1
            ? importName[0].getFullText()
            : importName[1].getFullText();
  ...
  getReferenceSourceFile(classe: ClassDeclaration, abstractName: string) {
    ...
    if (referenceClass) {
      const { exportAbstractName, importAbstractName, referenceSourceFile } =
        referenceClass;
      return {
        abstractName,
        importAbstractName,
  ...
  getFindReferenceClass(classe: ClassDeclaration, abstractName: string) {
    ...
    for (const referencedSymbol of referencedSymbols) {
      for (const reference of referencedSymbol.getReferences()) {
        ...
        const importDeclaration = referenceNode.getFirstAncestorByKind(
        ...
        if (importDeclaration !== null) {
          ...
          if (
            referenceNode
              .getSourceFile()
              .getFullText()
              .search(`implements ${abstractName}`) > 0
          ) {
            const exportAbstractName = referenceNode
              .getFullText()
              .replace(/\s*/g, "");
            // 存在一种情况
            // 导出的时候用了as,引用时候用了as.
   ...
   */
  addReferenceAbstract(
    abstract: string,
    exportAbstract: string,
    importAbstract: string
  ) {
  ...
  isImplementsInFile(classList: ClassDeclaration[], abstractName: string) {
    for (const classe of classList) {
      const heritageClause = classe.getChildrenOfKind(
  ...
  getInjectClassList() {
    ...
    for (const referenceFile of referenceFileList) {
      ...
      for (const exportClass of exportedDeclarations.keys()) {
        ...
        const heritageClauseList = exportedDeclaration[0].getChildrenOfKind(
          SyntaxKind.HeritageClause
        );
        if (heritageClauseList.length > 0) {
          const heritageClause = heritageClauseList[0]
            ...
            .getFullText();
          if (heritageClause) {
            const exportedDeclarationAbsName = heritageClause.replace(
              /\s*/g,
              ""
            );
  ...
  writeMainSourceFile() {
    ...
    const lastImport = this.mainSource
    ...
    for (const injectClass of this.injectClassList) {
      importClass.push({
        ...
        moduleSpecifier: this.mainSource.getRelativePathAsModuleSpecifierTo(
          injectClass.sourceFile
        ),
      });
    ...
    this.mainSource.insertStatements(
      this.mainSource.getImportDeclarations().length,
      inject
    );

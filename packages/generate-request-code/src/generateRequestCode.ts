...
export class GenerateRequestCode {
  public options: Omit<OptionsType, 'plugins' | 'requestConfig' | 'interceptorRequest' | 'interceptorResponse'> = {
    baseFilePath: '',
    domains: []
  }
  ...
  async run() {
    ...
    for (const domain of domains) {
      ...
      for (const template of templateList) {
        FileUtil.generateFile(baseFilePath + template.directory, template.fileName, template.content)
      }
